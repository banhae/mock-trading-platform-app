...
이 문서는 `exchange-app` 리포에 위치하지만, 작업 범위는 `exchange-app` /
`exchange-gitops` / `exchange-infra` 3개 리포 모두를 포함한다.
...
- exchange-app `ci.yaml`에 `ci-frontend` job 추가
- exchange-gitops에 `frontend` Application + dev values 추가
- exchange-infra `ecr.tf`에 `frontend` ECR 리포 추가
...
### Phase F1 — 인프라 준비 (exchange-infra)
...
### Phase F2 — 프론트엔드 소스 스캐폴드 (exchange-app)
...
### Phase F3 — Helm chart 추가 (exchange-app)
...
### Phase F5 — GitOps 연결 (exchange-gitops)
...
1. `exchange-app/README.md`
2. `exchange-app/SYSTEM_BLUEPRINT.md`
3. `exchange-gitops/README.md` 및 `SYSTEM_BLUEPRINT.md`
4. `exchange-infra/SYSTEM_BLUEPRINT.md`
...
| 리포 | 신규 파일 | 수정 파일 | 비고 |
|------|-----------|-----------|------|
| exchange-app | `services/frontend/*`, `charts/frontend/*` | `ci.yaml`, `README.md`, `SYSTEM_BLUEPRINT.md` | 프론트 앱 + chart + CI |
| exchange-gitops | `argocd/applications/dev/frontend.yaml`, `environments/dev/values/frontend.yaml` | `README.md`, `SYSTEM_BLUEPRINT.md` | Application + values |
| exchange-infra | - | `envs/dev/ecr.tf`, `SYSTEM_BLUEPRINT.md` | ECR repo 추가 |
...
2. **ALB Ingress 추가**
   - exchange-app에 ingress 템플릿
   - exchange-gitops에 ALB controller wiring

3. **ACM + Route53 + 커스텀 도메인**
   - exchange-infra에 인증서/도메인 리소스 추가
