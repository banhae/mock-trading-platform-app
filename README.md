...
**AWS EKS 기반 Mock Trading Platform 아키텍처** — Terraform · EKS · Helm · ArgoCD 기반 Cloud Ops 포트폴리오의 애플리케이션 리포지토리.

본 프로젝트는 금융/증권 서비스의 주문·계정·잔고·시세 흐름을 단순화한 뒤, IaC–CI/CD–GitOps로 이어지는 Kubernetes 운영 흐름을 설계·구현·검증하기 위해 구성했습니다.

이 리포는 3개 리포 중 하나로, 서비스 코드·Helm chart·CI 파이프라인을 담당합니다.
